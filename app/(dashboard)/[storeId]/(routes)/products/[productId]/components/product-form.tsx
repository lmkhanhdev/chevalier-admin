...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const [selectedSizes, setSelectedSizes] = useState<string[]>(
    initialData ? initialData.size.split(",") : []
  );
  ...
  const handleSizeButtonClick = (size: string) => {
    setSelectedSizes((prevSelectedSizes) => {
      if (prevSelectedSizes.includes(size)) {
        ...
        // Nếu size chưa được chọn, thêm nó vào danh sách
  ...
  const handleSubmit = () => {
    form.setValue("size", selectedSizes.join(",")); // Cập nhật giá trị của trường "sizeId"
  };
              ...
              render={({ field }) => (
                <FormItem>
                  <div>
                    ...
                    <input
                      type="text"
                      className="w-full h-[45px] border-[1px] rounded-lg focus:border-black px-3 focus:border-2 outline-none"
                      name="size"
                      value={field.value}
                      onChange={(e) => {
                        field.onChange(e);
                        setSelectedSizes(e.target.value.split(",")); // Split size string and update selectedSizes
                      }}
                    />
                  </div>
                  <div>
                    ...
                    <button
                      ...
                      type="button"
                      className="border-[0.5px] rounded-lg text-center text-[16px] py-[2px] cursor-pointer px-3 mt-4 mb-5 mr-5 bg-blue-500 text-white"
                    >
                      Submit
                    </button>
                  </div>
